...
if(world){
 ...
 if(stage&&scenes.length){
  scenes.forEach((scene,index)=>{scene.dataset.worldId||=`scene-${index+1}`;scene.dataset.worldLabel||=`Page ${index+1}`;});
  const layout=world.dataset.worldLayout||world.dataset.projectWorld||'identity',persistentNav=world.dataset.worldNav==='persistent';
  const nav=document.createElement('nav');nav.className=`world-nav world-nav--${layout}`;nav.dataset.worldNav='';nav.hidden=!persistentNav;nav.setAttribute('aria-label',persistentNav?`${layout} navigation`:'Project scenes');
  const links=scenes.map((scene,index)=>{const a=document.createElement('a'),number=document.createElement('span'),label=document.createElement('strong'),media=scene.querySelector('img,video');a.href=`#${scene.dataset.worldId}`;a.setAttribute('aria-label',`${String(index+1).padStart(2,'0')} — ${scene.dataset.worldLabel||`Scene ${index+1}`}`);number.textContent=String(index+1).padStart(2,'0');label.textContent=scene.dataset.worldLabel||`Scene ${index+1}`;if(media){const thumb=document.createElement('img');thumb.src=media.tagName==='VIDEO'?media.poster:(media.currentSrc||media.src);thumb.alt='';thumb.loading='lazy';a.append(thumb);}a.append(number,label);nav.append(a);return a;});
  const chapterSpecs=(world.dataset.worldChapters||'').split(';').map(entry=>{const [label,start]=entry.split(':');return {label,start:Number(start)-1};}).filter(chapter=>chapter.label&&Number.isInteger(chapter.start)&&chapter.start>=0&&chapter.start<scenes.length);
  const chapterNav=chapterSpecs.length?document.createElement('nav'):null,chapterLinks=[];
  if(chapterNav){chapterNav.className='world-chapters';chapterNav.setAttribute('aria-label','Publication chapters');chapterSpecs.forEach(chapter=>{const link=document.createElement('a');link.href=`#${scenes[chapter.start].dataset.worldId}`;link.textContent=chapter.label;chapterNav.append(link);chapterLinks.push(link);});}
  ...
  stage.append(nav);if(chapterNav)stage.append(chapterNav);stage.append(controls,focusBar);
  ...
  function render(){index=requestedIndex();document.body.dataset.activeScene=String(index+1);scenes.forEach((scene,i)=>{scene.hidden=i!==index;if(i!==index)scene.querySelectorAll('video').forEach(video=>video.pause());});links.forEach((link,i)=>i===index?link.setAttribute('aria-current','true'):link.removeAttribute('aria-current'));chapterLinks.forEach((link,i)=>{const next=chapterSpecs[i+1]?.start??scenes.length;if(index>=chapterSpecs[i].start&&index<next)link.setAttribute('aria-current','true');else link.removeAttribute('aria-current');});const active=scenes[index],source=active.querySelector('img'),video=active.querySelector('video'),artwork=active.querySelector('.archive-image-link');const backdrop=source?.currentSrc||source?.src||video?.poster;if(backdrop)stage.style.setProperty('--world-image',`url("${backdrop}")`);const label=`${String(index+1).padStart(2,'0')} / ${String(scenes.length).padStart(2,'0')} — ${active.dataset.worldLabel||`Scene ${index+1}`}`;status.textContent=label;status.ariaLabel=`Scene overview: ${label}`;focusStatus.textContent=label;if(artwork)original.href=artwork.href;world.dataset.worldReady='';}
  function setFocus(active,restore=true){document.body.classList.toggle('world-focus',active);focusBar.hidden=!active;delete document.body.dataset.worldZoomed;zoom.ariaLabel='Zoom artwork';zoom.textContent='Zoom';[document.querySelector('.window-bar'),world.querySelector('.archive-breadcrumb'),world.querySelector('.world-intro'),world.querySelector('.project-context'),nav,chapterNav].forEach(node=>{if(node)node.inert=active;});if(active)closeFocus.focus({preventScroll:true});else if(restore)focusOpener?.focus({preventScroll:true});}
  ...
  links.forEach((link,i)=>link.addEventListener('click',event=>{if(event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return;event.preventDefault();if(!persistentNav)nav.hidden=true;status.setAttribute('aria-expanded','false');go(i);}));
  chapterLinks.forEach((link,i)=>link.addEventListener('click',event=>{if(event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return;event.preventDefault();go(chapterSpecs[i].start);}));
  ...
  status.addEventListener('click',()=>{if(persistentNav)return;nav.hidden=!nav.hidden;status.setAttribute('aria-expanded',String(!nav.hidden));});
